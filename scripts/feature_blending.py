...
from runtime_spec import EAR_CLASS_INDEX, HAIR_CLASS_INDEX, scaled_mask_morph_kernel
...
def hairstyle_feature_blending(generator, seg, src_latent, src_feature, visual_mask, latent_bald, latent_global=None, latent_local=None, local_blending_mask=None):
    feature_size = src_feature.shape[-2:]
    mask_kernel = scaled_mask_morph_kernel(int(visual_mask.shape[-1]))
    device = src_feature.device
    feature_dtype = src_feature.dtype
    ...
    if latent_global is not None:
        ...
        ear_mask = torch.where(visual_mask == EAR_CLASS_INDEX, torch.ones_like(visual_mask), torch.zeros_like(visual_mask)).squeeze().cpu().numpy()
        hair_mask = torch.where(visual_mask == HAIR_CLASS_INDEX, torch.ones_like(visual_mask), torch.zeros_like(visual_mask)).squeeze().cpu().numpy()
        hair_ear_mask = ear_mask + hair_mask
        bald_blending_mask = dliate_erode(hair_ear_mask.astype('uint8'), mask_kernel)
        bald_blending_mask = torch.from_numpy(bald_blending_mask).unsqueeze(0).unsqueeze(0).to(device=device, dtype=feature_dtype)
        bald_blending_mask_down = F.interpolate(bald_blending_mask, size=feature_size, mode='bicubic')
        src_feature = bald_feature * bald_blending_mask_down + src_feature * (1 - bald_blending_mask_down)

        global_hair_mask = torch.where(global_proxy_seg == HAIR_CLASS_INDEX, torch.ones_like(global_proxy_seg), torch.zeros_like(global_proxy_seg))
        global_hair_mask_down = F.interpolate(global_hair_mask.to(dtype=feature_dtype), size=feature_size, mode='bicubic')
    ...
    if latent_local is not None:
        ...
        local_blending_mask = torch.from_numpy(local_blending_mask[:, :, 0]).unsqueeze(0).unsqueeze(0).to(device=device)
        ...
        local_blending_mask_down = F.interpolate(local_blending_mask.to(dtype=feature_dtype), size=feature_size, mode='bicubic')
...
def color_feature_blending(generator, seg, edited_hairstyle_img, src_latent, color_latent_in, latent_F):
    ...
    hair_mask = torch.where(hair_seg == HAIR_CLASS_INDEX, torch.ones_like(hair_seg), torch.zeros_like(hair_seg))
    mask_kernel = scaled_mask_morph_kernel(int(edited_hairstyle_img.shape[-1]))
    enlarged_hair_mask_np = dliate_erode(hair_mask[0][0].cpu().numpy().astype('uint8'), mask_kernel)
    enlarged_hair_mask = torch.from_numpy(enlarged_hair_mask_np).unsqueeze(0).unsqueeze(0).to(device=edited_hairstyle_img.device)
    final_hair_mask = F.interpolate(enlarged_hair_mask.float(), size=edited_hairstyle_img.shape[-2:]).long().clone().detach()
    ...
    final_hair_mask_down = F.interpolate(final_hair_mask.float(), size=source_feature.shape[-2:], mode='bicubic')
