...
class BicubicDownSample(nn.Module):
    ...
    def __init__(self, factor=4, padding='reflect'):
        ...
        self.register_buffer("k1", torch.cat([k1, k1, k1], dim=0), persistent=False)
        k2 = torch.reshape(k, shape=(1, 1, 1, size))
        self.register_buffer("k2", torch.cat([k2, k2, k2], dim=0), persistent=False)
    ...
    def forward(self, x, nhwc=False, clip_round=False, byte_output=False):
        ...
        filter_dtype = x.dtype if torch.is_floating_point(x) else torch.float32
        filters1 = self.k1.to(device=x.device, dtype=filter_dtype)
        filters2 = self.k2.to(device=x.device, dtype=filter_dtype)
